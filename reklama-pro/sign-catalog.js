document.addEventListener('DOMContentLoaded', () => {
  ...
  sections.forEach((sectionData, sectionIndex) => {
    ...
    sectionData.items.forEach(item => {
      ...
      card.innerHTML = `<button class="poster-card__preview" type="button" aria-label="Открыть страницу ${item[1]}"><img src="../reklama-2/assets/znaki/${item[0]}" alt="${sectionData.title}, страница ${item[1]}" loading="lazy"></button><div class="poster-card__number"><div class="poster-card__page"><span>Страница</span><strong>${item[1]}</strong></div><div class="poster-card__codes"><span>Коды знаков</span><b>${item[2]}</b></div></div>`;
  ...
  function show(index) {
    ...
    viewerImage.src = `../reklama-2/assets/znaki/${item.image}`;
